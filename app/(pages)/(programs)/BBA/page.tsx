...
// Import JSON Data
import specializationsData from "@/utils/data/BBA/bbaSpecialization.json";
import skillsData from "@/utils/data/Skills.json";
import placement from "@/utils/data/BBA/bbaPlacement.json";
import bbaData from "@/utils/data/BBA/bbaData.json";
...
const iconMap: Record<string, React.ElementType> = {
  ...
  userCog: UserCog,
  fileCheck: FileCheck,
  badgeCheck: BadgeCheck,       // <-- Added
  clipboardList: ClipboardList, // <-- Added
  fileEdit: FileEdit ,
    form: FileText,
...
const page = () => {
 ...
 useGSAP(() => {
    // 1. Standard Entrance animations for all elements with .gsap-fade-up
    ...
    // 2. NEW: Staggered animation for the Admissions cards
    gsap.fromTo('.gsap-admission-card', 
      { y: 50, opacity: 0 }, 
      { 
        y: 0, 
        opacity: 1, 
        duration: 0.8, 
        stagger: 0.15, // This is the magic! Each card delays by 0.15s
        ease: 'power3.out', 
        scrollTrigger: { 
          trigger: '.admissions-grid-container', // Trigger when the grid wrapper enters
          start: 'top 80%' 
        } 
      }
    );
    ...
    >
      ...
      <section className="px-4 pt-16 pb-12 text-center sm:px-8 lg:px-24">
        <h1 className="gsap-fade-up text-3xl font-extrabold tracking-tight text-foreground sm:text-5xl lg:text-6xl">
          {bbaData.hero.titlePrefix}{" "}
          <span className="text-primary">{bbaData.hero.titleHighlight}</span>
        </h1>
        <p className="gsap-fade-up mx-auto mt-8 max-w-4xl text-lg text-muted-foreground sm:text-xl leading-relaxed">
          {bbaData.hero.descStart}{" "}
          <strong className="text-foreground">{bbaData.hero.descBold}</strong>
          {bbaData.hero.descEnd}
        </p>
      </section>
        ...
        <div className="mx-auto max-w-360 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="flex flex-col">
            <h2 className="gsap-fade-up text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl mb-8">
              {bbaData.whyChoose.title}
            </h2>

            <div className="gsap-fade-up bg-card p-6 sm:p-8 rounded-2xl border border-border shadow-sm">
              <h3 className="text-xl font-bold text-foreground mb-6">
                {bbaData.whyChoose.subtitle}
              </h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {bbaData.whyChoose.benefits.map((benefit, index) => (
                  <li
                    key={index}
            ...
            <p className="gsap-fade-up mt-8 text-muted-foreground leading-relaxed text-justify">
              {bbaData.whyChoose.description}
            </p>

            {bbaData.whyChoose.buttonText && (
              <button 
                className="gsap-fade-up mt-8 bg-primary text-background px-8 py-4 font-bold rounded-full hover:bg-foreground/90 transition-colors cursor-pointer" 
        ...
        <div className="mx-auto max-w-360">
          <div className="gsap-fade-up text-center max-w-5xl mx-auto mb-16">
            <h2 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl lg:text-5xl mb-6">
              Our BBA Programs/Specializations
            </h2>
            <p className="text-lg text-muted-foreground leading-relaxed">
              Our BBA program comes with 7 specializations. We welcome a diverse
              range of interested students for the BBA with our specializations.
              BBA specializations equip the latest knowledge according to
              industry standards. Along with that, we provide 30+ skill-based
              value-added certifications to improve overall students&apos;
              growth.
            </p>
          </div>
      ...
      <section className="w-full bg-slate-50 dark:bg-slate-900/40 px-4 py-16 sm:px-8 sm:py-24 lg:px-24 border-t border-border/50">
        <div className="mx-auto max-w-360">
          <div className="gsap-fade-up text-center max-w-4xl mx-auto mb-16">
            ...
            <p className="text-lg text-muted-foreground leading-relaxed">
              Making you skilled is our ultimate goal with our BBA programs.
      ...
      <section className="w-full bg-primary px-4 py-16 sm:px-8 sm:py-24 lg:px-24">
        <div className="mx-auto max-w-360">
          <h2 className="gsap-fade-up mb-16 text-center text-3xl font-extrabold tracking-tight text-primary-foreground sm:text-4xl lg:text-5xl">
            {bbaData.eligibility.title}
          </h2>

          <div className="grid grid-cols-1 gap-12 md:grid-cols-3">
            {bbaData.eligibility.items.map((item, index) => {
    ...
    <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-center lg:items-start">
      ...
      <div className="flex w-full flex-col justify-center pt-8 lg:sticky lg:top-24 lg:w-4/12">
        <h2 className="gsap-fade-up mb-6 text-4xl font-extrabold leading-tight tracking-tight text-foreground sm:text-5xl">
          {bbaData.admissionsProcess.title}
        </h2>
        <p className="gsap-fade-up text-lg leading-relaxed text-muted-foreground">
          {bbaData.admissionsProcess.description}
        </p>
      </div>

      <div className="w-full lg:w-8/12">
        <div className="admissions-grid-container grid grid-cols-1 gap-6 sm:grid-cols-2 lg:gap-8">
          {bbaData.admissionsProcess.steps.map((step, index) => {
            const IconComponent = iconMap[step.icon];
            const isHighlight = step.highlight;
            
            const bgClass = isHighlight ? 'bg-primary text-primary-foreground border-transparent' : 'bg-card text-foreground border-border';
            const textClass = isHighlight ? 'text-primary-foreground' : 'text-foreground';
            const mutedTextClass = isHighlight ? 'text-primary-foreground/90' : 'text-muted-foreground';
            const iconBorderClass = isHighlight ? 'border-primary-foreground/20' : 'border-foreground/10';
            
            const hoverShadow = isHighlight ? 'hover:shadow-primary/40' : 'dark:hover:shadow-primary/5';
            ...
            return (
              ...
              >
                <div className={`mb-6 flex h-20 w-20 items-center justify-center rounded-full border-2 transition-transform duration-300 group-hover:-rotate-3 group-hover:scale-110 ${iconBorderClass} ${textClass}`}>
                  <IconComponent className="h-10 w-10 transition-transform duration-300" strokeWidth={1.5} />
                </div>
